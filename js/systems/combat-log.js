/* ================================================================
   ...
   - 产物 / ISK / LP 不逐帧记录，而是在打开日志时，用 resetCombatRunState 开局拍下的「库存快照」
     与当前状态做差（仅取净获得），天然覆盖在线+离线。
   - 技能经验**不计快照差**：改为钩住战斗唯一经验入口 addStationModifiedCombatXp，仅累计「战斗授予」
     的每技能经验（含在线+离线），从而**排除空间站授予的非战斗经验**。
   - ISK/LP 也来自库存差（避免事件里 LP 双计：死亡空间每波 LP 与全通 LP 已含关系）。
   ================================================================ */
...
function ensureCombatRunLog() {
  ...
  if (!combat.runLog || typeof combat.runLog !== "object") {
    ...
    const rl = {
      ...
      isk: 0, lp: 0,
      invStartSnapshot: (typeof createInventorySnapshot === "function") ? createInventorySnapshot(gameState) : null,
      startSkills: {},
      skillXp: {}
    };
    if (gameState.skills) {
...
function getCombatLog() {
  ...
  // 库存快照差 → ISK / LP / 产物（在线+离线合并，仅取净获得）
  let isk = 0, lp = 0;
  const loot = [];
  if (rl.invStartSnapshot && typeof createInventorySnapshot === "function" && typeof diffInventorySnapshot === "function") {
    let diff = diffInventorySnapshot(rl.invStartSnapshot, createInventorySnapshot(gameState));
    if (Array.isArray(diff)) {
      if (typeof splitOfflineDispatchBonus === "function") diff = splitOfflineDispatchBonus(diff);
      for (const it of diff) {
        if (!it || typeof it.id !== "string") continue;
        const qty = Number(it.quantity) || 0;
        if (qty <= 0) continue;
        if (it.id === "currency:isk") { isk += qty; continue; }
        if (it.id === "currency:lp") { lp += qty; continue; }
        loot.push(it);
      }
    }
  }
...
if (typeof GameEvents !== "undefined" && GameEvents && typeof GameEvents.on === "function") {
  ...
  GameEvents.on("combat:deathspaceWaveCleared", function () { combatLogInc("dsWaves", 1); });
  GameEvents.on("combat:deathspaceCleared", function () { combatLogInc("dsClears", 1); });
