...
// rareWeights: 稀有发现内部权重 {blueprint, booster, probe, credential}（PLAN §7）。
//   2026-08-28 废弃两个键位：implant（考古脑插改为事件监听独立掉落，不再占稀有池）与
//   extractorSmall（考古直接发小型脑突触提取剂与「重复脑插转化」定位冲突）；结构中直接移除。
...
const ARCHAEOLOGY_LOCATIONS = Object.freeze([
  {
    ...
    rareRate: 0.020,
    rareWeights: { blueprint:20, booster:60, probe:0, credential:0 },
  ...
  },
  {
    ...
    rareRate: 0.015,
    rareWeights: { blueprint:15, booster:45, probe:25, credential:5 },
  ...
  },
  {
    ...
    rareRate: 0.010,
    rareWeights: { blueprint:15, booster:40, probe:25, credential:5 },
  ...
  },
  {
    ...
    rareRate: 0.0075,
    rareWeights: { blueprint:15, booster:40, probe:25, credential:0 },
  ...
  },
  {
    ...
    rareRate: 0.005,
    rareWeights: { blueprint:20, booster:40, probe:20, credential:0 },
