(function () {
  ...
  function skip(element) { return !element || /^(SCRIPT|STYLE|CODE|PRE)$/.test(element.tagName) || !!element.closest?.('#achievements-panel, #starmap-panel, [data-deferred-i18n]'); }
  ...
  function setLocale(next) { if (!supported.includes(next)) return; locale = next; localStorage.setItem(STORAGE_KEY, next); applyNav(); var control = document.getElementById("setting-language"); if (control) control.value = locale; window.dispatchEvent(new CustomEvent("localechange", { detail: { locale: locale } })); }
